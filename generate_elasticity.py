...
def generate_single_video(elasticity_value, initial_conditions, num_frames=64, width=224, height=224):
    space = pymunk.Space()
    space.gravity = (0, 0)
    ...
    shape.friction = 0.0
    shape.elasticity = float(elasticity_value)
    space.add(body, shape)

    for seg_pts in [((0, 0), (width, 0)), ((0, height), (width, height)),
                    ((0, 0), (0, height)), ((width, 0), (width, height))]:
        seg = pymunk.Segment(space.static_body, seg_pts[0], seg_pts[1], 1)
        seg.elasticity = 1.0
        seg.friction = 0.0
        space.add(seg)
...
def generate_dataset(
    ...
    output_dir="data/generated6.0/elasticity",
    dataset_seed=1337,
):
    ...
    for _ in range(num_scenarios):
        ...
        v0 = float(rng.uniform(400.0, 800.0))
        scenarios.append(
            {
                "x": float(rng.uniform(radius + 5, width - radius - 5)),
                "y": float(rng.uniform(radius + 5, height - radius - 5)),
